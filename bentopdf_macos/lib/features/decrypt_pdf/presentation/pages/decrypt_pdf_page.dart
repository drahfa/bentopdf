...
class DecryptPdfPage extends ConsumerWidget {
  ...
  Widget _buildErrorBanner(WidgetRef ref, DecryptPdfState state) {
    return Container(
      ...
      child: Row(
        children: [
          ...
          IconButton(
            ...
            onPressed: () => ref.read(decryptPdfProvider.notifier).clearError(),
          ),
        ],
      ),
    );
  ...
  Widget _buildSuccessBanner(WidgetRef ref, DecryptPdfState state) {
    return Container(
      margin: const EdgeInsets.all(16),
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(
      ...
      child: Row(
        children: [
          ...
          IconButton(
            icon: const Icon(Icons.close, size: 18),
            color: PdfEditorTheme.muted,
            onPressed: () => ref.read(decryptPdfProvider.notifier).clearSuccess(),
          ),
        ],
      ),
    );
  ...
  Widget _buildOptionsCard(BuildContext context, WidgetRef ref, DecryptPdfState state) {
    return Container(
      decoration: BoxDecoration(
        ...
        children: [
          Row(
            children: [
              Container(
                ...
                child: const Icon(Icons.lock_open, size: 18, color: PdfEditorTheme.accent),
              ),
              const SizedBox(width: 12),
              const Text(
                'Enter Password',
          ...
          TextField(
            ...
            decoration: InputDecoration(
              ...
              prefixIcon: Icon(Icons.key, color: PdfEditorTheme.muted),
            ),
            onChanged: (value) => ref.read(decryptPdfProvider.notifier).setPassword(value),
            onSubmitted: (_) {
                ...
                child: Row(
                  ...
                  children: [
                    if (state.isProcessing)
                      ...
                      const Icon(Icons.lock_open, size: 18, color: PdfEditorTheme.text),
                    const SizedBox(width: 10),
                    Text(
